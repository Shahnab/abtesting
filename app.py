...
st.set_page_config(
    page_title="A/B Test Application"
)
...
st.write(
    """
# A/B Test App
Upload experiment results to see the significance of A/B test
"""
)
...
if uploaded_file:
    ...
    with st.form(key="my_form"):
        ab = st.multiselect(
            "A/B column",
            options=df.columns,
            help="Select which column refers to A/B testing labels.",
            default=ab_default,
        )
        if ab:
            ...
            decide = st.radio(
                ...
                options=["Yes", "No"],
                help="Select yes if this is group B (or the treatment group) from test.",
            )
            if decide == "No":
